...
public class SearchResult_Controller {
    ...
    void initialize() {
        ...
        SearchButton.setOnAction(event -> {
            String find = (String)(SearchField.getText());
            Table.getItems().clear();
            for(int i = 0;i< Room.toys.size();i++)
            {
                if(find.equals(Room.toys.get(i).getName()))
                {       toyData.add(Room.toys.get(i));
                ...
                else if(find.equals(Double.toString(Room.toys.get(i).getCost())))
                {
                    toyData.add(Room.toys.get(i));
                    ToyName.setCellValueFactory(new PropertyValueFactory<Toy,String>("name"));
                    ToyType.setCellValueFactory(new PropertyValueFactory<Toy,String>("type"));
                ...
                else if(find.equals(Room.toys.get(i).getType()))
                {
                    toyData.add(Room.toys.get(i));
                    ToyName.setCellValueFactory(new PropertyValueFactory<Toy,String>("name"));
                    ToyType.setCellValueFactory(new PropertyValueFactory<Toy,String>("type"));
                ...
                else if(find.equals(Room.toys.get(i).getSize_category()))
                {
                    toyData.add(Room.toys.get(i));
                    ToyName.setCellValueFactory(new PropertyValueFactory<Toy,String>("name"));
                    ToyType.setCellValueFactory(new PropertyValueFactory<Toy,String>("type"));
